...
- 템플릿(Template)
	- 템플릿에 대한 이해와 함수 템플릿
		- 함수 템플릿의 특수화(Specialization)
			- 템플릿 함수의 구성 방법에 예외를 둘 필요가 있을 경우 사용하는 것
			- 함수의 다형성과 재활용적인 측면을 제공(함수 오버로딩은 다형성은 만족시키지만 재활용적인 측면에서는 부족)
			- 삼항 연산자(ternary operator) : 조건식 ? 반환값1 : 반환값2
			- 물음표(?) 앞의 조건식에 따라 결괏값이 참(true)이면 반환값1을 반환하고, 결괏값이 거짓(false)이면 반환값2를 반환
	- 클래스 템플릿(Class Template)
		- 클래스의 일반화된 선언
		- 클래스 템플릿 기반의 객체 생성에는 반드시 자료형 정보를 명시해야함.
		- 선언과 정의를 분리할 경우, 정의부분에도 템플릿 선언을 일일히 해야함.
				
## 14일차(23.03.11)
- 예외처리
	- try, catch문
- Vector
	- 배열의 단점 : 자료구조가 연속적이기 때문에 수정이 힘들고 배열의 크기를 바꿀 수 없음
	- vector : 자동으로 메모리가 할당 -> 알아서 새로운 배열을 만들어주고 중간에 삭제하면 알아서 앞으로 당겨줌 -> 배열의 단점을 보완
	- vector을 사용하려면 템플릿을 활용해야함
	- 첫번째 방은 vector.front, 맨마지막 방은 vector.back으로 vector의 앞뒤에 접근
	- pop_back : 맨 뒤의 값을 꺼내는 것을
	- push_back : vector의 맨 뒤에 값을 삽입
	- 헤더파일 <vector>와 usingnamespace가 필요